...
DatagrumpSender::DatagrumpSender(const char *const host, const char *const port,
                                 const bool debug)
    : socket_(), controller_(debug), sequence_number_(0),
...
void DatagrumpSender::got_ack(const uint64_t timestamp,
  ...
  next_ack_expected_ =
      max(next_ack_expected_, ack.header.ack_sequence_number + 1);

  /* Inform congestion controller */
  controller_.ack_received(ack.header.ack_sequence_number,
                           ack.header.ack_send_timestamp,
                           ack.header.ack_recv_timestamp, timestamp);
}
...
void DatagrumpSender::send_datagram(const bool after_timeout) {
  ...
  controller_.datagram_was_sent(cm.header.sequence_number,
                                cm.header.send_timestamp, after_timeout);
}
...
int DatagrumpSender::loop() {
  ...
  poller.add_action(Action(
      socket_, Direction::Out,
      [&]() {
        /* Close the window */
